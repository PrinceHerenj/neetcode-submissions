class SegmentTree {
    struct Node {
        ...
        Node* left;
        Node* right;
        Node(int total, int L, int R): sum(total), L(L), R(R), left(nullptr), right(nullptr) {}
    };
public:
    Node* root;
    ...
    int query_helper(Node* root, int L, int R) {
        ...
        if (R <= M) {
            ...
            return query_helper(root->right, L, R);
        } else {
            return query_helper(root->left, L, M) + query_helper(root->right, M + 1, R);
        }
    }
};
